  ...
  <div class="container mx-auto p-10">
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      <div v-for="request in requests" :key="request.id"
        class="-backdrop-hue-rotate-60 hover:backdrop-hue-rotate-60 bg-white/30 shadow-2xl rounded-md">
        ...
        <div class="p-6 flex flex-col">
          ...
          <button @click="handleEdit(request.id)" data-test="edit-button"
            class="bg-cyan-400 text-white py-2 px-6 rounded-md shadow-sm hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-amber-500 mt-3">Edit</button>
          <button @click="handleDelete(request.id)" data-test="delete-button"
            class="bg-amber-600 text-white py-2 px-6 rounded-md shadow-sm hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-red-500 mt-3">Delete</button>
        </div>
      </div>
    </div>
  </div>
